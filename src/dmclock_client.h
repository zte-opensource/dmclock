...
namespace crimson {
  namespace dmclock {
    ...
    // request and the global rho gets incremented when a response
    // comes back noting it was handled by reservation. Depending on
    // the interleaving of requests and reponses, rho may increase a
    // lot between increments of delta for a given server, so in order
    // to keep rho no more than delta, we pool are excess rho and use
    // it opportunistically. We may want to consider removing the
    // contraint that rho is never more than delta.
    class AltTracker {
      Counter delta_prev_req;
      Counter rho_prev_req;
      Counter rho_pool;       // rho we have not yet consumed
      
    public:

      AltTracker(Counter _delta_prev_req,
	...
	return AltTracker(the_delta, the_rho);
      ...
      inline ReqParams prepare_req(Counter& the_delta, Counter& the_rho) {
	++the_delta;
	// since we increment the_delta here, we don't need to increment by one
	Counter delta_out = the_delta - delta_prev_req;
	// we only increment rho when we get a response back saying
	// the request was handled via reservation; to avoid a zero
	// rho value, we always increment here
	Counter rho_out = rho_pool + 1 + the_rho - rho_prev_req;

	// if we cannot use the entire pool, use as much as possible
	// and save the rest for later
	if (rho_out > delta_out) {
	  rho_pool = rho_out - delta_out;
	  rho_out = delta_out;
	} else {
	  rho_pool = 0;
	}
